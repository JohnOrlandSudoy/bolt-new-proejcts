...
const FeatureCard = ({ 
  ...
  return (
    <motion.div
      ...
      className={`group relative rounded-2xl bg-gradient-to-br from-white/5 to-white/[0.02] backdrop-blur-sm border border-white/10 hover:border-cyan-400/30 transition-all duration-300 hover:shadow-xl hover:shadow-cyan-500/10 ${isMobile ? 'p-4' : 'p-6'}`}
    >
      ...
      <div className="relative mb-4">
        <div className={`${isMobile ? 'w-10 h-10' : 'w-12 h-12'} rounded-xl bg-gradient-to-br from-cyan-500/20 to-purple-500/20 flex items-center justify-center group-hover:scale-110 transition-transform duration-300`}>
          <Icon className={`${isMobile ? 'size-5' : 'size-6'} text-cyan-400 group-hover:text-cyan-300 transition-colors`} />
        </div>
      </div>
      
      {/* Content */}
      <div className="relative z-10">
        <h3 className={`${isMobile ? 'text-base' : 'text-lg'} font-semibold text-white mb-2 group-hover:text-cyan-100 transition-colors`}>
          {title}
        </h3>
        <p className={`text-gray-400 ${isMobile ? 'text-xs' : 'text-sm'} leading-relaxed group-hover:text-gray-300 transition-colors`}>
          {description}
        </p>
      </div>
...
const StatItem = ({ 
  ...
  return (
    ...
    >
      <div className={`${isMobile ? 'text-2xl' : 'text-3xl'} font-bold bg-gradient-to-r from-cyan-400 to-purple-400 bg-clip-text text-transparent mb-1`}>
        {value}
      </div>
      <div className={`${isMobile ? 'text-xs' : 'text-sm'} text-gray-400 font-medium`}>{label}</div>
    </motion.div>
  );
};
      ...
      <main className="relative z-20 flex flex-col min-h-screen">
        ...
        <div className="flex-1 flex flex-col items-center justify-center px-4 sm:px-6 py-20 text-center">
          ...
          >
            ...
            <div className="relative">
              ...
              <img 
                ...
                alt="NyxtGen Logo" 
                className={`relative drop-shadow-2xl ${isMobile ? 'w-16 h-16' : 'w-20 h-20'}`}
              />
            </div>
            ...
            <div className="space-y-2">
              <h1 
                className={`${isMobile ? 'text-4xl sm:text-5xl' : 'text-6xl sm:text-7xl md:text-8xl lg:text-9xl'} font-black bg-gradient-to-r from-white via-cyan-100 to-purple-200 bg-clip-text text-transparent tracking-tight leading-none`}
                style={{ 
              ...
              <motion.div
                ...
                className={`flex items-center justify-center gap-3 text-cyan-300 ${isMobile ? 'text-base' : 'text-xl'} font-semibold`}
              >
                <Sparkles className={`${isMobile ? 'size-4' : 'size-6'} animate-pulse`} />
          ...
          <motion.div
            ...
            className={`space-y-4 sm:space-y-6 mb-8 sm:mb-12 max-w-5xl ${isMobile ? 'px-2' : ''}`}
          >
            <h2 className={`${isMobile ? 'text-2xl sm:text-3xl' : 'text-4xl sm:text-5xl md:text-6xl'} font-bold text-white/95 leading-tight`}>
              Experience the{" "}
            ...
            <p className={`${isMobile ? 'text-base sm:text-lg' : 'text-xl sm:text-2xl md:text-3xl'} text-gray-300 leading-relaxed font-light max-w-4xl mx-auto`}>
          ...
          <motion.div
            ...
            className={`flex ${isMobile ? 'flex-col' : 'flex-col sm:flex-row'} items-center justify-center gap-4 sm:gap-6 mb-12 sm:mb-16 w-full max-w-md sm:max-w-none`}
          >
            {/* Primary CTA */}
            <AudioButton
              onClick={handleGetStarted}
              className={`group relative overflow-hidden ${isMobile ? 'w-full' : ''} px-8 sm:px-12 py-4 sm:py-6 ${isMobile ? 'text-lg' : 'text-xl'} font-bold text-white bg-gradient-to-r from-cyan-500 to-blue-600 hover:from-cyan-400 hover:to-blue-500 rounded-2xl shadow-2xl shadow-cyan-500/25 hover:shadow-3xl hover:shadow-cyan-500/40 transition-all duration-300 hover:scale-105 border border-cyan-400/20`}
            >
              {/* Animated background */}
              ...
              <span className="relative z-10 flex items-center gap-3 justify-center">
                <Play className={`${isMobile ? 'size-5' : 'size-6'} fill-current`} />
                Start Experience
                <ArrowRight className={`${isMobile ? 'size-5' : 'size-6'} group-hover:translate-x-1 transition-transform`} />
              </span>
          ...
          <motion.div
            ...
            className={`flex items-center justify-center ${isMobile ? 'gap-8' : 'gap-12'} mb-12 sm:mb-16 p-4 sm:p-6 rounded-2xl bg-white/5 backdrop-blur-sm border border-white/10 max-w-md sm:max-w-none`}
          >
        ...
        <motion.div 
          style={{ y: y1 }}
          className="px-4 sm:px-6 py-16 bg-gradient-to-t from-black/80 to-transparent"
        >
          <div className="max-w-7xl mx-auto">
            ...
            >
              <h3 className={`${isMobile ? 'text-2xl sm:text-3xl' : 'text-4xl'} font-bold text-white mb-4`}>
                Powered by{" "}
        ...
        >
          <div className="flex items-center justify-center gap-2 mb-4">
            {[...Array(5)].map((_, i) => (
              <Star key={i} className={`${isMobile ? 'size-4' : 'size-5'} text-yellow-400 fill-current`} />
            ))}
          </div>
          <blockquote className={`${isMobile ? 'text-lg' : 'text-2xl'} font-light text-gray-300 italic max-w-4xl mx-auto`}>
