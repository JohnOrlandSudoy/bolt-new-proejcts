  ...
  return (
    <div className="relative group">
      ...
      <input
        type={type}
        className={cn(
          "flex h-16 w-full rounded-xl border-2 border-slate-500 bg-slate-900 px-4 py-4 text-base text-white placeholder:text-slate-400 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-400 focus-visible:border-cyan-400 transition-all duration-200 font-mono shadow-lg",
          icon && "pl-12",
          showToggle && "pr-14",
      ...
      {showToggle && (
        <button
          type="button"
          onClick={onToggle}
          className="absolute right-4 top-1/2 -translate-y-1/2 text-cyan-400 hover:text-cyan-300 transition-colors z-10 p-2 rounded-lg hover:bg-white/10"
        >
...
>(({ className, ...props }, ref) => {
  return (
    <div className="relative group">
      <textarea
        className={cn(
          "flex min-h-[140px] w-full rounded-xl border-2 border-slate-500 bg-slate-900 px-4 py-4 text-base text-white placeholder:text-slate-400 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-400 focus-visible:border-cyan-400 transition-all duration-200 resize-none font-mono shadow-lg hover:border-cyan-500/70 hover:bg-slate-800",
          className
        )}
        style={{ 
          fontFamily: "'Source Code Pro', monospace",
...
>(({ className, icon, children, ...props }, ref) => {
  return (
    <div className="relative group">
      ...
      <select
        className={cn(
          "flex h-16 w-full rounded-xl border-2 border-slate-500 bg-slate-900 px-4 py-4 text-base text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-400 focus-visible:border-cyan-400 transition-all duration-200 appearance-none cursor-pointer font-mono shadow-lg hover:border-cyan-500/70 hover:bg-slate-800",
          icon && "pl-12",
          className
        )}
        style={{ 
      ...
      <div className="absolute right-4 top-1/2 -translate-y-1/2 pointer-events-none">
        <svg className="w-6 h-6 text-cyan-400 group-focus-within:text-cyan-300 transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
const SettingsSection = ({ 
  ...
  return (
    <div className="space-y-8 p-8 rounded-2xl bg-slate-800/60 backdrop-blur-sm border-2 border-slate-600/50 hover:border-slate-500/70 transition-all duration-200 shadow-xl">
      <div className="flex items-start gap-5">
        <div className="p-4 rounded-xl bg-gradient-to-r from-cyan-500/30 to-blue-500/30 border-2 border-cyan-500/40 flex-shrink-0 shadow-lg">
...
const HelpText = ({ children }: { children: React.ReactNode }) => {
  return (
    <p className="text-sm text-slate-400 leading-relaxed mt-2 px-1">
      {children}
    </p>
  );
};
    ...
    <DialogWrapper>
      <AnimatedTextBlockWrapper>
        <div className="relative w-full max-w-6xl mx-auto">
          {/* Enhanced Header */}
          <div className="sticky top-0 pt-8 pb-8 z-20 bg-gradient-to-b from-black/95 to-black/80 backdrop-blur-lg border-b border-slate-600/30">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-5">
                <div className="p-4 rounded-xl bg-gradient-to-r from-cyan-500/30 to-blue-500/30 border-2 border-cyan-500/40 shadow-lg">
                  <Sparkles className="size-8 text-cyan-300" />
                </div>
                <div>
                  <h1 className="text-4xl font-bold text-white mb-2">Settings</h1>
                  <p className="text-lg text-slate-300">Customize your AI conversation experience</p>
                </div>
              </div>
              <Button
                variant="ghost"
          ...
          <div className="max-h-[calc(100vh-320px)] overflow-y-auto pr-4 space-y-10 py-6">
            ...
            <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-6">
              <div className="text-sm text-slate-300 leading-relaxed space-y-2">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <span className="font-bold text-white text-base">Settings Tips</span>
                </div>
